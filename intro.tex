...
\jn{The relationship of the four projects are not very clear. Maybe we can say something more ambitious. For example, we can say that ``the sampling idea used for AQP opened up a new research direction; we believe the new SampleClean idea can do the same thing. We applied this idea to many domains, and found that it can address many challenging problems that are very hard to solve before. After that, we can introduce our stale view cleaning and ActiveClean work in the MV and ML settings to demonstrate how powerful this idea is. In the end, we can talk about the system a little bit."}
...
\jn{Would be good to add a paragraph to describe how the following sections are organized}