\section{The Seminal Work on SampleClean \cite{wang1999sample}}


\jn{This is the same comment as the one that I gave in the Abstract, but I haven't figured out a better way to solve it. I changed the title of this section to address this problem. Please feel free to change it if you don't like it.}
...
\jn{Do you think that we need to mention importance sampling somewhere?}

\subsubsection{Attribute Errors}
This sampling statistics change only affected duplication errors, attributes errors only affects an individual row. \jn{something wrong with the sentence.}
...
In other words, the probability that a given tuple is sampled is not changed by our correction of value and condition errors \jn{value and condition errors?}, thus we define $\saqpplusfunc(t)$ as:
\[\small
...
To clean a sample of publications, we first manually removed the mis-attributions in the sample. Then, we applied the technique used in~\cite{DBLP:journals/pvldb/WangKFF12} \jn{the citation is missing} to identify potential duplicates for all of publications in our sample, and manually examined the potential matches.  
...
In Figure \ref{exp:ms-academic-ranking}, we plot the probability of incorrect \reminder{correct?} ranking as a function of number of cleaned records with SampleClean.
...
\caption{Even though the dirty result always returns an incorrect ranking, we can return the correct ranking with 95\% probability after cleaning only 210 total samples. To achieve a correct ranking with 99\% probability, we require 326 samples to be cleaned.\label{exp:ms-academic-ranking} \jn{enlarge font size}}
