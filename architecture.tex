...
In View Cleaning, we extend this set of queries to any queries that can be estimated with a statistical bootstrap \cite{agarwalknowing}. \jn{Please doublecheck this sentence. Based on my understanding, the statistical bootstrap is for getting bounds, not for estimating query results. }
...
4) A hybrid plan that sends only difficult webpages to the crowd, maximizing accuracy without sacrificing latency. \reminder{I didn't found the reference to this figure in the text. }}
\label{fig:ex-plan}
\end{SCfigure}
